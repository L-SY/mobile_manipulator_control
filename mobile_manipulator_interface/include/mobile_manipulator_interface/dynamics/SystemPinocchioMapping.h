...
#include <mobile_manipulator_interface/dynamics/Dimensions.h>
...
class TripleIntegratorPinocchioMapping final : public ocs2::PinocchioStateInputMapping<Scalar>
{
public:
  ...
  std::pair<MatXs, MatXs> getOcs2Jacobian(const VecXs& state, const MatXs& Jq, const MatXs& Jv) const override
  {
...
template <typename Scalar>
class NonholonomicPinocchioMapping final : public ocs2::PinocchioStateInputMapping<Scalar>
{
public:
  using VecXs = VecX<Scalar>;
  using MatXs = MatX<Scalar>;
...
template <typename Scalar>
static const TripleIntegratorPinocchioMapping<Scalar> OBSTACLE_PINOCCHIO_MAPPING{ OBSTACLE_DIMENSIONS };

template <typename Mapping, typename Scalar>
class SystemPinocchioMapping final : public ocs2::PinocchioStateInputMapping<Scalar>
{
public:
  using VecXs = VecX<Scalar>;
  using MatXs = MatX<Scalar>;

  explicit SystemPinocchioMapping(const OptimizationDimensions& dims) : dims_(dims), robot_mapping_(dims.robot)
  {
  }
  ...
  VecXs getPinocchioJointPosition(const VecXs& state) const override
  {
    ...
    for (int i = 0; i < dims_.o; ++i)
    {
      ...
      q_pin.segment(i * 3, 3) = OBSTACLE_PINOCCHIO_MAPPING<Scalar>.getPinocchioJointPosition(x_obs);
    }

    // Then we add on the robot q
    VecXs x_robot = state.head(dims_.robot.x);
    q_pin.tail(dims_.robot.q) = robot_mapping_.getPinocchioJointPosition(x_robot);
  ...
  VecXs getPinocchioJointVelocity(const VecXs& state, const VecXs& input) const override
  {
    ...
    {
      VecXs x_obs = state.segment(dims_.robot.x + i * 9, 9);
      v_pin.segment(i * 3, 3) = OBSTACLE_PINOCCHIO_MAPPING<Scalar>.getPinocchioJointVelocity(x_obs, u_obs);
    }

    // Then we add on the robot v
    VecXs x_robot = state.head(dims_.robot.x);
    VecXs u_robot = input.head(dims_.robot.u);
    v_pin.tail(dims_.robot.v) = robot_mapping_.getPinocchioJointVelocity(x_robot, u_robot);
  ...
  VecXs getPinocchioJointAcceleration(const VecXs& state, const VecXs& input) const override
  {
    ...
    {
      VecXs x_obs = state.segment(dims_.robot.x + i * 9, 9);
      a_pin.segment(i * 3, 3) = OBSTACLE_PINOCCHIO_MAPPING<Scalar>.getPinocchioJointAcceleration(x_obs, u_obs);
    }

    // Then we add on the robot a
    VecXs x_robot = state.head(dims_.robot.x);
    VecXs u_robot = input.head(dims_.robot.u);
    a_pin.tail(dims_.robot.v) = robot_mapping_.getPinocchioJointAcceleration(x_robot, u_robot);
  ...
  // dfdu.
  std::pair<MatXs, MatXs> getOcs2Jacobian(const VecXs& state, const MatXs& Jq_pin, const MatXs& Jv_pin) const override
  {
    ...
    for (int i = 0; i < dims_.o; ++i)
    {
      ...
      std::tie(dfdx_obs, std::ignore) =
          OBSTACLE_PINOCCHIO_MAPPING<Scalar>.getOcs2Jacobian(x_obs, Jq_pin_obs, Jv_pin_obs);
    ...
    std::tie(dfdx_robot, dfdu_robot) = robot_mapping_.getOcs2Jacobian(x_robot, Jq_pin_robot, Jv_pin_robot);
