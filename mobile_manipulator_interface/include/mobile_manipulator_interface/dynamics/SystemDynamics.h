...
public:
  ...
  VecX<Scalar> flowmap(Scalar t, const VecX<Scalar>& x, const VecX<Scalar>& u, const VecX<Scalar>& p) const
  {
...
public:
  ...
  VecX<Scalar> flowmap(Scalar t, const VecX<Scalar>& x, const VecX<Scalar>& p) const
  {
    VecX<Scalar> dxdt(9);
...
{
public:
  explicit SystemDynamics(const std::string& model_name, const OptimizationDimensions& dims,
                          const std::string& model_folder = "/tmp/ocs2", bool recompile_libraries = true,
                          bool verbose = true)
    : dims_(dims), robot_dynamics_(dims.robot), ocs2::SystemDynamicsBaseAD()
  {
    initialize(dims.x(), dims.u(), model_name, model_folder, recompile_libraries, verbose);
  }
  ...
  VecXad systemFlowMap(ocs2::ad_scalar_t time, const VecXad& state, const VecXad& input,
