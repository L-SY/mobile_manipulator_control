...
{
public:
  EndEffectorCost(const MatXd& W, const ocs2::EndEffectorKinematics<ocs2::scalar_t>& kinematics)
    : W_(W), kinematics_ptr_(kinematics.clone())
  {
    if (kinematics.getIds().size() != 1)
    {
      throw std::runtime_error("[EndEffectorConstraint] endEffectorKinematics has wrong "
  ...
  ocs2::scalar_t getValue(ocs2::scalar_t time, const VecXd& state, const ocs2::TargetTrajectories& target,
                                                                       ...
                                                                       const ocs2::PreComputation&) const override
  {
    ...
    const auto eeOrientationError =
        kinematics_ptr_->getOrientationErrorLinearApproximation(state, { desired_pose.second }).front();
...
{
public:
  EndEffectorCostCppAd(const MatXd& W, const ocs2::PinocchioEndEffectorKinematicsCppAd& kinematics,
    ...
    : W_(W), kinematics_ptr_(kinematics.clone()), dims_(dims)
  {
    ...
    initialize(dims.x(), num_params, "end_effector_cost_cppad", "/tmp/ocs2", recompileLibraries, true);
  }
...
protected:
  ocs2::ad_scalar_t costFunction(ocs2::ad_scalar_t time_ad, const VecXad& state, const VecXad& parameters) const
  {
    ...
    Vec3ad orn_error = kinematics_ptr_->getOrientationErrorCppAd(state, desired_orientation);
