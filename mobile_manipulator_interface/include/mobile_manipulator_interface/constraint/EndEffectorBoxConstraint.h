...
public:
    ...
    , reference_manager_ptr_(&reference_manager)
  {
    ...
    {
      throw std::runtime_error("[EndEffectorBoxConstraint] kinematics has wrong "
                               "number of end effector IDs.");
    }
    if (xyz_lower.rows() != 3)
    {
      throw std::runtime_error("[EndEffectorBoxConstraint] lower bound must be of length 3.");
    }
    if (xyz_upper.rows() != 3)
    {
      throw std::runtime_error("[EndEffectorBoxConstraint] upper bound must be of length 3.");
    }
  }
  ...
  {
    return new EndEffectorBoxConstraint(xyz_lower_, xyz_upper_, *kinematics_ptr_, *reference_manager_ptr_);
  }
