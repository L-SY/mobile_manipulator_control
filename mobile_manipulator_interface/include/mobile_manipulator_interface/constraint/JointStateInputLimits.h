...
public:
  JointStateInputLimits(const OptimizationDimensions& dims)
    : ocs2::StateInputConstraint(ocs2::ConstraintOrder::Linear), dims_(dims)
  {
  }
  ...
  // limit)
  VecXd getValue(ocs2::scalar_t time, const VecXd& state, const VecXd& input, const ocs2::PreComputation&) const override
  {
  ...
  {
    ocs2::VectorFunctionLinearApproximation limits(getNumConstraints(time), state.rows(), input.rows());
...
{
public:
  JointStateInputConstraint(const OptimizationDimensions& dims, const VecXd& state_limit_lower,
                            const VecXd& state_limit_upper, const VecXd& input_limit_lower,
    ...
    : ocs2::StateInputConstraint(ocs2::ConstraintOrder::Linear), dims_(dims)
  {
    ...
    e_ = VecXd::Zero(n);
    e_ << -state_limit_lower, state_limit_upper, -input_limit_lower, input_limit_upper;
  }
  ...
  ocs2::VectorFunctionLinearApproximation getLinearApproximation(ocs2::scalar_t time, const VecXd& state,
                                                                 const VecXd& input, const ocs2::PreComputation&) const
  {
