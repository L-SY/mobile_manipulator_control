...
class NominalBalancingConstraints final : public ocs2::StateInputConstraintCppAd
{
public:
  EIGEN_MAKE_ALIGNED_OPERATOR_NEW

  NominalBalancingConstraints(const ocs2::PinocchioEndEffectorKinematicsCppAd& pinocchioEEKinematics,
  ...
  NominalBalancingConstraints* clone() const override
  {
    ...
    return new NominalBalancingConstraints(*pinocchioEEKinPtr_, settings_, gravity_, dims_, false);
  }
...
private:
  NominalBalancingConstraints(const NominalBalancingConstraints& other) = default;

  std::unique_ptr<ocs2::PinocchioEndEffectorKinematicsCppAd> pinocchioEEKinPtr_;
...
class ContactForceBalancingConstraints final : public ocs2::StateInputConstraintCppAd
{
public:
  EIGEN_MAKE_ALIGNED_OPERATOR_NEW

  ContactForceBalancingConstraints(const ocs2::PinocchioEndEffectorKinematicsCppAd& pinocchioEEKinematics,
                                   const BalancingSettings& settings, const Vec3d& gravity,
                                   const OptimizationDimensions& dims, bool recompileLibraries);
  ...
  ContactForceBalancingConstraints* clone() const override
  {
    // Always pass recompileLibraries = false to avoid recompiling the same
    // library just because this object is cloned.
    return new ContactForceBalancingConstraints(*pinocchioEEKinPtr_, settings_, gravity_, dims_, false);
  }
...
private:
  ContactForceBalancingConstraints(const ContactForceBalancingConstraints& other) = default;
...
public:
  ...
  ObjectDynamicsConstraints(const ocs2::PinocchioEndEffectorKinematicsCppAd& pinocchioEEKinematics,
                            const BalancingSettings& settings, const Vec3d& gravity, const OptimizationDimensions& dims,
  ...
  ObjectDynamicsConstraints* clone() const override
  {
    // Always pass recompileLibraries = false to avoid recompiling the same
    // library just because this object is cloned.
    return new ObjectDynamicsConstraints(*pinocchioEEKinPtr_, settings_, gravity_, dims_, false);
  }
