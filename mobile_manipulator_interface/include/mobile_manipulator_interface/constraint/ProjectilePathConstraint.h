...
#include <mobile_manipulator_interface/common/ReferenceTrajectory.h>
...
ocs2::scalar_t cubic_newtons(ocs2::scalar_t a, ocs2::scalar_t b, ocs2::scalar_t c, ocs2::scalar_t d, ocs2::scalar_t x0,
                             ocs2::scalar_t tol)
{
  int max_iter = 10;
...
ocs2::scalar_t projectile_closest_time(const Vec3d& r, const Vec3d& r0, const Vec3d& v0, const Vec3d& g,
...
{
public:
  ProjectilePathConstraint(const ocs2::EndEffectorKinematics<ocs2::scalar_t>& kinematics,
    ...
    , scale_(scale)
  {
    if (kinematics.getIds().size() != distances.size())
    {
      ...
      throw std::runtime_error("Number of distances and EEs must be equal!");
    }
  }
  ...
  {
    return new ProjectilePathConstraint(*kinematics_ptr_, *reference_manager_ptr_, distances_, scale_);
  }
  ...
  VecXd getValue(ocs2::scalar_t time, const VecXd& state, const ocs2::PreComputation&) const override
  {
  ...
  ocs2::VectorFunctionLinearApproximation getLinearApproximation(ocs2::scalar_t time, const VecXd& state,
                                                                 const ocs2::PreComputation& pre_comp) const override
  {
    ...
    const size_t nc = getNumConstraints(time);
    auto approximation = ocs2::VectorFunctionLinearApproximation(nc, state.rows(), 0);
    approximation.setZero(nc, state.rows(), 0);

    for (int i = 0; i < nc; ++i)
    {
      const auto position_approx = kinematics_ptr_->getPositionLinearApproximation(state)[i];
      ...
      MatXd A_obs(3, 9);
      A_obs << Mat3d::Identity(), dt * Mat3d::Identity(), 0.5 * dt * dt * Mat3d::Identity();

      // closest point on the obstacle's predicted future path
      Vec3d r_closest = A_obs * x_obs;

      Vec3d delta = r_ew_w - r_closest;
      Vec3d normal = delta.normalized();  // TODO: 0 when in contact---bad Jacobian?
      ...
      approximation.dfdx.row(i) << w * s * normal.transpose() * (position_approx.dfdx - dobs_dx);
    }
...
private:
  ...
  VecXd distances_;
  std::unique_ptr<ocs2::EndEffectorKinematics<ocs2::scalar_t>> kinematics_ptr_;
