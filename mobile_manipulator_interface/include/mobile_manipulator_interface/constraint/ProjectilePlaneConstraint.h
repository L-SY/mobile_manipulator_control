...
{
public:
  ProjectilePlaneConstraint(const ocs2::EndEffectorKinematics<ocs2::scalar_t>& kinematics,
                            const ocs2::ReferenceManager& reference_manager, ocs2::scalar_t distance)
    ...
    , distance_(distance)
  {
    ...
    {
      throw std::runtime_error("[ProjectilePlaneConstraint] kinematics has wrong "
  ...
  {
    return new ProjectilePlaneConstraint(*kinematics_ptr_, *reference_manager_ptr_, distance_);
  }
                                                                 ...
                                                                 const ocs2::PreComputation& pre_comp) const override
  {
    ...
    // zero.
    auto approximation = ocs2::VectorFunctionLinearApproximation(1, state.rows(), 0);
    ...
    const auto position_approx = kinematics_ptr_->getPositionLinearApproximation(state).front();
