...
#include <mobile_manipulator_interface/common/Types.h>
#include "mobile_manipulator_interface/constraint/BoundedBalancingConstraints.h"
...
std::ostream& operator<<(std::ostream& out, const BalancingSettings& settings)
{
  out << "enabled = " << settings.enabled << std::endl
      << "normal enabled = " << settings.constraints_enabled.normal << std::endl
      << "friction enabled = " << settings.constraints_enabled.friction << std::endl
...
RigidBodyState<ocs2::ad_scalar_t>
get_rigid_body_state(const std::unique_ptr<ocs2::PinocchioEndEffectorKinematicsCppAd>& kinematics_ptr,
                     const VecXad& state, const VecXad& input)
{
  ...
  X.acceleration.angular = kinematics_ptr->getAngularAccelerationCppAd(state, input);
  return X;
}

NominalBalancingConstraints::NominalBalancingConstraints(
    const ocs2::PinocchioEndEffectorKinematicsCppAd& pinocchioEEKinematics, const BalancingSettings& settings,
    const Vec3d& gravity, const OptimizationDimensions& dims, bool recompileLibraries)
  ...
  , dims_(dims)
{
  ...
  {
    throw std::runtime_error("[TrayBalanaceConstraint] endEffectorKinematics has wrong "
  ...
  initialize(dims.x(), dims.u(), 0, "upright_nominal_balancing_constraints", "/tmp/ocs2", recompileLibraries, true);
}

VecXad NominalBalancingConstraints::constraintFunction(ocs2::ad_scalar_t time, const VecXad& state, const VecXad& input,
                                                       const VecXad& parameters) const
{
  RigidBodyState<ocs2::ad_scalar_t> X = get_rigid_body_state(pinocchioEEKinPtr_, state, input);

  BalancedObjectArrangement<ocs2::ad_scalar_t> ad_arrangement = arrangement_.cast<ocs2::ad_scalar_t>();
  ...
  , dims_(dims)
{
  ...
  {
    num_constraints_ = settings_.contacts.size() * NUM_LINEARIZED_FRICTION_CONSTRAINTS_PER_CONTACT;
  }

  // compile the CppAD library
  const std::string lib_name = "upright_contact_force_constraints";
  initialize(dims.x(), dims.u(), 0, lib_name, "/tmp/ocs2", recompileLibraries, true);
}

VecXad ContactForceBalancingConstraints::constraintFunction(ocs2::ad_scalar_t time, const VecXad& state,
  ...
  , dims_(dims)
{
  ...
  num_constraints_ = settings_.objects.size() * NUM_DYNAMICS_CONSTRAINTS_PER_OBJECT;
...
VecXad ObjectDynamicsConstraints::constraintFunction(ocs2::ad_scalar_t time, const VecXad& state, const VecXad& input,
                                                     const VecXad& parameters) const
{
  // All forces are expressed in the EE frame
  VecXad forces = input.tail(dims_.f());
  ...
  return compute_object_dynamics_constraints(ad_objects, ad_contacts, forces, X, ad_gravity) / n;
}
